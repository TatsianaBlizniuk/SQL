...
--СПИСОК ВСЕХ СОТРУДНИКОВ И НОМЕРА ЗАКАЗОВ, КОТРЫЕ ОНИ ВЫПОЛНЯЛИ
...
--SELECT * FROM ORDERS
SELECT CUST, COUNT(CUST) AS КОЛ_ЗАК
FROM ORDERS
GROUP BY CUST

--3.21. Подсчитать итоговую сумму заказа для каждого покупателя.
SELECT CUST, SUM(AMOUNT) AS ИТОГОВАЯ_СУММА
FROM ORDERS
GROUP BY CUST

--3.22. Подсчитать среднюю цену заказа для каждого сотрудника.
--SELECT * FROM ORDERS
SELECT REP, AVG(AMOUNT) AS СУММА_ЗАКАЗА_КАЖДОГО_СОТРУДНИКА
FROM ORDERS
GROUP BY REP
...
SELECT MFR_ID, count(PRODUCT_ID) AS количество_продуктов_для_каждого_производителя
FROM PRODUCTS
GROUP BY MFR_ID

--3.25. Найти самый дорогой товар каждого производителя.
--SELECT * FROM PRODUCTS
SELECT MFR_ID, MAX(PRICE) as САМЫЙ_ДОРОГОЙ 
FROM PRODUCTS 
GROUP BY MFR_ID;
